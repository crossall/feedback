...
const basePath = "project_expo/storage/shared/";

function safeKey(value: unknown) {
  const key = String(value ?? "").trim();
  if (!key || key.length > 240) throw new Error("Invalid storage key.");
  return key;
}
...
export async function POST(request: Request) {
  try {
    ...
    console.error("Project expo storage request failed:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Project expo storage request failed." },
      { status: 500 },
    );
  }
}
