...
const PROJECT1_FACT_CHECK_INSTRUCTION =
...
const PROJECT1_WEB_SEARCH_TOOL = {
...
function messagesWithFactCheck(messages: AnthropicMessage[]) {
  ...
  return messages.map((message, index) => {
    ...
    if (typeof message.content === "string") {
      return {
        ...message,
        content: `${message.content}\n\n${PROJECT1_FACT_CHECK_INSTRUCTION}`,
      };
    }
    return {
      ...message,
      content: [
        ...message.content,
        { type: "text" as const, text: PROJECT1_FACT_CHECK_INSTRUCTION },
      ],
    };
  });
}
...
export async function POST(request: Request) {
  try {
    ...
    const apiKey = process.env.PROJECT1_ANTHROPIC_API_KEY?.trim() || apiKeys.anthropic;
    if (!apiKey) {
    ...
    const claudePayload = {
      ...
      ...(shouldUseWebSearch ? { tools: [PROJECT1_WEB_SEARCH_TOOL] } : {}),
    };
    ...
    console.error("Project1 Claude request failed:", error);
    return NextResponse.json(
