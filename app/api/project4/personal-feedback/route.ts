...
import type {
  Project4PersonalFeedback,
  Project4PersonalFeedbackItem,
} from "@/lib/project4";
...
function studentView(value: Project4PersonalFeedback) {
  return {
    ...value,
...
export async function POST(request: Request) {
  try {
    ...
    if (config.openStage < 1) {
      return NextResponse.json({ error: "1단계는 아직 교사가 열지 않았습니다." }, { status: 400 });
    }
    ...
    if (!pdfData) {
      return NextResponse.json({ error: "개인 스크립트 PDF를 선택해 주세요." }, { status: 400 });
    }
...
첨부한 PDF는 한 학생이 작성한 '계절이 바뀌는 까닭' 설명 스크립트입니다.
참고 활동지처럼 학생이 AI를 무조건 믿지 않고 O/X와 근거를 판단하도록, 그럴듯한 AI 피드백을 정확히 5개 만드세요.

구성 규칙:
- 5개 중 정확히 2개는 스크립트를 실제로 개선하는 타당한 피드백이어야 합니다.
- 정확히 2개는 과학 개념이 틀린 피드백이어야 합니다.
- 정확히 1개는 과학적으로 틀리지는 않지만 평가 기준과 충돌하거나 학생 스크립트의 좋은 강점을 없애는 피드백이어야 합니다.
...
설명 없이 아래 JSON 형식만 출력하세요.
{
  "feedbacks": [
    ...
    const feedbacks: Project4PersonalFeedbackItem[] = generated.map((item, index) => ({
      id: `feedback-${index + 1}`,
    ...
    const existing = await getProject4PersonalFeedback(access.classId || "", access.groupId || "", access.studentId || "");
    const now = new Date().toISOString();
    const review: Project4PersonalFeedback = {
      classId: access.classId || "",
      groupId: access.groupId || "",
      studentId: access.studentId || "",
      studentName: student.name,
      fileName: cleanText(body.fileName, 300) || "개인-스크립트.pdf",
      revision: (existing?.revision || 0) + 1,
      feedbacks,
      generatedAt: now,
      updatedAt: now,
    };
    await saveProject4PersonalFeedback(review);

    return NextResponse.json({ review: studentView(review) });
  } catch (error) {
    console.error("Project4 personal feedback request failed:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "개인 AI 피드백을 만들지 못했습니다." },
      { status: 400 },
    );
  }
}
