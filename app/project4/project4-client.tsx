    ...
    <section className={styles.home}>
      ...
      <div className={styles.roleGrid}>
        ...
        <button type="button" onClick={() => setRole("juniorLogin")}>
          <span className={styles.roleIcon}><School size={25} /></span><div><small>5학년</small><strong>형님 영상 평가</strong><p>패들렛에서 영상을 본 뒤 이해한 정도를 알려 줘요.</p></div><ChevronRight />
        </button>
...
function StudentLogin({ config, onBack, onEnter }: { config: Project4Config; onBack: () => void; onEnter: (identity: { classId: string; groupId: string; studentName: string }) => Promise<void> }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(""); try { await onEnter({ classId, groupId, studentName }); } catch (caught) { setError(caught instanceof Error ? caught.message : "입장하지 못했습니다."); } finally { setBusy(false); } }
  return <CenteredPanel title="6학년 프로젝트 평가" subtitle="자기 반과 모둠을 고르고 이름을 직접 입력하세요." onBack={onBack}><form className={styles.loginForm} onSubmit={submit}><label><span>반</span><select value={classId} onChange={(event) => { setClassId(event.target.value); setGroupId(""); setStudentName(""); }}><option value="">반 선택</option>{config.classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span>모둠</span><select value={groupId} onChange={(event) => { setGroupId(event.target.value); setStudentName(""); }} disabled={!classId}><option value="">모둠 선택</option>{groups.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span>이름</span><input value={studentName} onChange={(event) => setStudentName(event.target.value)} disabled={!groupId} autoComplete="name" placeholder="명단에 등록된 이름" /></label><p className={styles.helper}>입력한 이름은 교사만 확인하며, 친구 화면의 평가 의견에는 표시되지 않습니다.</p>{error && <InlineError text={error} />}<button className={styles.primaryButton} disabled={busy || !classId || !groupId || !studentName.trim()}>{busy ? <Loader2 className={styles.spin} /> : <ChevronRight size={17} />} 평가 시작</button></form></CenteredPanel>;
}
...
function JuniorEvaluation({ token, onDone }: { token: string; onDone: () => void }) {
  ...
  if (done) return <CenteredPanel title="고마워요!" subtitle="솔직하게 알려 준 내용이 6학년 형님들의 다음 설명에 큰 도움이 됩니다." onBack={onDone}><button className={styles.primaryButton} onClick={onDone}><Check size={17} /> 처음으로</button></CenteredPanel>;
  return <section className={styles.juniorPage}><span className={styles.kicker}><School size={15} /> 5학년 후배 평가</span><h1>계절이 왜 바뀌는지<br />이해했나요?</h1><p>6학년 형님들이 지구본과 전등으로 실험한 결과를 바탕으로 설명 영상을 만들었습니다. 패들렛에서 영상을 보고 솔직하게 답해 주세요.</p><form className={styles.juniorForm} onSubmit={submit}><fieldset><legend>1. 계절이 왜 바뀌는지 이해했나요?</legend><div className={styles.choiceGrid}>{[["well", "잘 이해했어요"], ["some", "조금 알 것 같아요"], ["little", "잘 모르겠어요"]].map(([value, label]) => <button type="button" className={understanding === value ? styles.active : ""} key={value} onClick={() => setUnderstanding(value)}>{label}</button>)}</div></fieldset><fieldset><legend>2. 어느 부분이 가장 이해가 잘 됐나요? <small>여러 개 선택</small></legend><div className={styles.checkGrid}>{Object.entries(partLabels).map(([value, label]) => <button type="button" className={helpfulParts.includes(value) ? styles.active : ""} key={value} onClick={() => toggle(value)}>{helpfulParts.includes(value) && <Check size={15} />}{label}</button>)}</div></fieldset><label><span>3. 더 궁금한 것이나 이해되지 않은 것이 있나요? <small>선택</small></span><textarea rows={4} value={question} onChange={(event) => setQuestion(event.target.value)} /></label><label><span>형님들에게 한마디 남겨 주세요. <small>선택</small></span><textarea rows={3} value={message} onChange={(event) => setMessage(event.target.value)} /></label>{error && <InlineError text={error} />}<button className={styles.primaryButton} disabled={busy || !understanding}>{busy ? <Loader2 className={styles.spin} /> : <Send size={17} />} 평가 보내기</button></form></section>;
}
